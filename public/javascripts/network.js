		...
		$.ajax({
			...
			success: function(data) {
				...
				$(json).each(function (index, item) {
					nodeARR.push({data : { id : item.netID_regulator , name : item.regulator, snpeff : item.regulator_info.snpeff_unique, positions : item.regulator_info.variant_positions, muts : item.regulator_info.provean_mutations }, group:"nodes",removed:false,selected:false,selectable:true,locked:false,grabbed:false,grabbable:true})
					nodeARR.push({data : { id : item.netID_target, name : item.target, snpeff : item.target_info.snpeff_unique, positions : item.target_info.variant_positions, muts : item.target_info.provean_mutations }, group:"nodes",removed:false,selected:false,selectable:true,locked:false,grabbed:false,grabbable:true})
				...
				console.log("this is nodeARR:", nodeARR)
				console.log(edgeARR)
				$('#netTable').html(table)
				$('#cy').cytoscape({
					layout: {
